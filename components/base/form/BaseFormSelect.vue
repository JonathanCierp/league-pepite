    ...
    <v-popover ref="selectEl">
      ...
      <template slot="popover">
        <ul class="overflow-y-auto max-h-52 text-sm" :style="[popoverWidth]">
          ...
          >
            <BaseIcon v-if="option[itemKey] === modelValue || modelValue.includes(option[itemKey])" name="check" size="20" />
            <span :class="[option[itemKey] === modelValue || modelValue.includes(option[itemKey]) ? 'ml-4' : 'ml-9']">{{ option[itemValue] }}</span>
          </li>
        </ul>
      </template>
    </v-popover>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    onMounted(() => {
      popoverWidth.value = { width: `${selectEl.value?.$el.offsetWidth}px` }
    })
